...
- Use the function `DESeq()` to perform the differential expression analysis
  ...
  NOTE: you could choose an LRT or Wald test of significance at this workflow stage
...
```
Make and MAplot
use lfcShrinnk() first

```{r}
dds_shrink <- lfcShrink(dds_filtered_unique, coef = 2)
plotMA(dds_shrink) #with logfold change shrinkage - standard best practice
plotMA(dds_filtered_unique) #without logfold change shrinkage 
