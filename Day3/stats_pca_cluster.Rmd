...
```{r}
pca_iris_x <- pca_iris$x
plot( x= pca_iris_x[,1], y = pca_iris_x[,2])
```
...
What is variance in this context? The contribution to variance of each principle component is the square of the stdev of that component, divided by the sum of the squares of the standard deviation of ALL principle components
...
```
Summary function on the PCA data does the above calculation 
...
Cluster 1 - Setosa, Cluster 2 - Versicolor, Cluster 3 - Verginicor
...
Overlay (using colours) the clustering from the four methods
