...
```{r}
all_genes_df <- read.csv("dds_results.csv")
```
...
upreg_genes_df <- all_genes_df %>% filter(padj<0.05 & log2FoldChange > 1)

downreg_genes_df<- all_genes_df %>% filter(padj<0.05 & log2FoldChange < -1)
```
...
```{r}
all_gene_tested_ids <- all_genes_tested_df[,1]
upreg_gene_ids <- upreg_genes_df[,1]
downreg_gene_ids <- downreg_genes_df[,1]
```
...
results_filter <- 
  ...
  filter(term_size > 4 & term_size < 301
         )
...
Plot the filtered results table:

```{r}
gost_results_obj_filter <- gost_results_obj
gost_results_obj_filter$result <- results_filter

gostplot(gost_results_obj_filter)
...
```{r}
multi_gostquery_results_obj <- gost_results_obj <- gost(query = list("upreg_genes" = upreg_gene_ids,
...
```{r}
all_genes_tested_df_GSEA <- all_genes_df %>%  filter(!is.na(padj))
```
...
```{r}
all_genes_tested_df_GSEA <- all_genes_tested_df_GSEA[,c("gene_id","log2FoldChange" )]
head(all_genes_tested_df_GSEA)
```

- Check whether you have genes with multiple entries.

```{r}
all_genes_tested_df_GSEA<- distinct(all_genes_tested_df_GSEA)
```
...
```{r}
ranks <- deframe(all_genes_tested_df_GSEA)
```
...
```{r}
h_gene_sets <- msigdbr("Mus musculus",
                      category = "H" )
```
