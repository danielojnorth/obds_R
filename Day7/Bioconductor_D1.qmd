...
# note: the links this generates don't work on server. Opening with "vignette()" 
# solves this. Also works off server. 
```
...
```{r}
Ebox <- "CANNTG"
countPattern(pattern = Ebox, subject = chrY, fixed = FALSE   )
```
...
```{r}
gtf_data <- BiocIO::import(con = "/project/shared/r/1_r_data_science/8-bioconductor/Homo_sapiens.GRCh38.104.gtf.gz")
gtf_data
```
...
```{r}
n_distinct(gtf_ACTB$transcript_name, na.rm = TRUE)
...
gtf_ACTB_transcripts <- split(gtf_ACTB_exons, f = gtf_ACTB_exons$transcript_id)
...
- Use the `useMart()` function to create an object named `mart` using the `"ENSEMBL_MART_ENSEMBL"` BioMart database and set the host option to `"https://may2021.archive.ensembl.org"`.
  Why would one do that in practice? - reproducibility 
...
```{r}
assay_counts <- read.csv(
  "/project/shared/r/1_r_data_science/8-bioconductor/summarized_experiment/counts.csv", row.names = 1) 

assay_matrix <- as.matrix(assay_counts)
